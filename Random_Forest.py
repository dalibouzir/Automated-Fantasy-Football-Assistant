...
def train_model(players):
    # Convert players data to pandas DataFrame
    df = pd.DataFrame(players)

    # Select features and target
    X = df[['now_cost', 'total_points', 'form', 'minutes', 'goals_scored', 'assists', 'yellow_cards', 'team_name', 'next_opponent']]
    y = df['total_points']  # Assume total_points is what we're predicting for now

    # Preprocessing: Impute missing data and encode categorical variables
    preprocessor = ColumnTransformer(
        transformers=[
            ('num', SimpleImputer(strategy='mean'), ['now_cost', 'total_points', 'form', 'minutes', 'goals_scored', 'assists', 'yellow_cards']),
...
def predict_player_performance(model, players):
    # Prepare the player data for prediction
    df = pd.DataFrame(players)
    X = df[['now_cost', 'total_points', 'form', 'minutes', 'goals_scored', 'assists', 'yellow_cards', 'team_name', 'next_opponent']]
    
    # Predict points
    predicted_points = model.predict(X) / 10
...
def predict_best_squad():
    ...
    # Train model and predict performance
    model = train_model(PLAYERS)
    players_with_predictions = predict_player_performance(model, PLAYERS)
...
def api_view_saved_squads():
    saved_squads = list(saved_teams_collection.find({}, {"_id": 1, "squad": 1, "date_saved": 1, "total_cost": 1}))
